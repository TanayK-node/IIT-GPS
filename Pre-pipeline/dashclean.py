...
# =========================
st.set_page_config(page_title="High-Performance GPS Dashboard", layout="wide", page_icon="🚀")

# Default Paths
DEFAULT_ROADS_PATH = "../map/IITB_Auto_POV_Shapefile.gpkg"
DEFAULT_ID_MAP_PATH = "0_ID_name.csv"
...
@st.cache_resource
def load_maps_and_tree(roads_path):
    """Loads map data once and caches it."""
...
# --- WORKER FUNCTION (Pure Python Logic) ---
...
def process_matching_parallel(_df_seg, _tree, _geoms, lat_col, lon_col):
    ...
    # prefer="processes" bypasses Python GIL for true parallelism.
    # Note: This requires Shapely >= 2.0 to pickle the STRtree efficiently.
    results_lists = Parallel(n_jobs=-1, prefer="processes")(
...
st.title("🚀 High-Performance Map Matcher & Cleaner")
st.markdown("Upload raw GPS files to clean, match, and calculate status using **multi-core processing**.")

# --- SIDEBAR CONFIG ---
with st.sidebar:
    st.header("⚙️ Configuration")
    roads_path = st.text_input("Road Network Path (.gpkg)", value=DEFAULT_ROADS_PATH)
    id_map_path = st.text_input("ID Mapping File (.csv)", value=DEFAULT_ID_MAP_PATH)
    
    st.divider()
    if os.path.exists(roads_path):
        st.success("✅ Road Network Found")
    else:
        st.error("❌ Road Network Missing")
        
    if os.path.exists(id_map_path):
        st.success("✅ ID Map Found")

# --- MAIN INTERFACE ---
uploaded_files = st.file_uploader("Upload CSV Files", type=["csv"], accept_multiple_files=True)

if st.button("🚀 Process Files (Using All Cores)", type="primary", disabled=not uploaded_files):
    
    with st.spinner("Loading Map Network..."):
        tree, geoms = load_maps_and_tree(roads_path)
        id_map = None
        if os.path.exists(id_map_path):
    ...
    if tree is None:
        st.error("Could not load Road Network.")
        st.stop()
            ...
            try:
                ...
                if 'driverId' in df.columns:
                    if id_map is not None:
                        ...
                        df['Driver_Name'] = "Unknown"
                else:
                    st.warning(f"Skipped {uploaded_file.name}: Missing 'driverId'")
                    continue

                # C. Timestamp & Format (Strict HH:MM:SS)
                ...
                df['IST_time'] = df['timestamp_ist'].dt.strftime('%H:%M:%S') # Fix 0-24 bug
                ...
                # E. Parallel Matching
                if lat_col not in df.columns: continue
                matched_df = process_matching_parallel(df, tree, geoms, lat_col, lon_col)
                
                if matched_df.empty:
                    st.warning(f"No matches in {uploaded_file.name}")
                    continue

                # F. Status & Speed Calculation
                ...
                matched_df['speed'] = np.where(matched_df['dt'] > 0, matched_df['ds'] / matched_df['dt'], 0)
                ...
                csv_buffer = final_output.to_csv(index=False)
                zf.writestr(f"matched_{uploaded_file.name}", csv_buffer)
                processed_count += 1
    ...
    status_text.success(f"Processing Complete! ({processed_count} files)")
    
    if processed_count > 0:
        st.download_button(
            label="💾 Download ZIP",
            data=zip_buffer.getvalue(),
            file_name="optimized_gps_output.zip",
